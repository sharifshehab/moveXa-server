...
const approveParcel = async (parcelId: string, decodedToken: JwtPayload) => {
    ...
    if (parcel.currentStatus === ParcelStatus.CANCELLED || parcel.currentStatus === ParcelStatus.BLOCKED) {
        throw new AppError(StatusCodes.NOT_FOUND, `Parcel is ${parcel.currentStatus}`);
    }
    if (parcel.payment !== PAYMENT_STATUS.PAID) {
        throw new AppError(StatusCodes.NOT_FOUND, `Payment is ${parcel.payment}`);
    }
